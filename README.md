...
rétroplanning), tâches consultants, reporting automatisé, journal, paramètres —
et **scouting commercial** : carte des boulangeries/pâtisseries OpenStreetMap
de toute la Belgique, zones d'exclusion autour des concurrents forts, zones
prioritaires, fiche d'implantation avec CA estimé (modèle de l'étude Halle).
...
- **Frontend** : HTML/CSS/JS vanilla (ES modules, zéro build). Une SPA
  (`public/index.html` + `public/assets/js/`) qui reproduit le prototype à
  l'identique. Aucune donnée métier dans le HTML : tout vient de l'API
  (contrat dans `docs/contrat-api.md`). Seule bibliothèque tierce : Leaflet
  1.9.4, embarquée dans `public/assets/vendor/leaflet/` (aucun CDN) et chargée
  uniquement à l'ouverture de l'écran Scouting.
...
public/
  ...
  assets/
    ...
    css/app.css         styles applicatifs (scrollbars, animations, hover, carte)
...
`/pwa/reports`, `/scouting`, `/scouting/tiles/{secteur}`.
...
`PUT /parametres/{key}`, `POST /journal`, et pour le scouting :
`PUT /scouting/tiles/{secteur}`, `PUT /scouting/competitors`,
`POST /scouting/candidates`, `DELETE /scouting/candidates/{id}`,
`PUT /scouting/populations`.
